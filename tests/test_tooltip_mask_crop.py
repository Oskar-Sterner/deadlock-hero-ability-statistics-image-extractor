...
def test_build_tooltip_image_applies_transparent_alpha_outside_polygon():
    ...
    assert tooltip_image.mode == "RGBA"
    assert tooltip_image.size == (8, 8)

    assert tooltip_image.getpixel((0, 0))[3] == 0
    assert tooltip_image.getpixel((3, 3))[3] == 255
...
def test_build_tooltip_image_without_polygon_is_opaque():
    ...
    assert tooltip_image.getchannel("A").getextrema() == (255, 255)


def test_build_tooltip_image_with_components_preserves_gaps_as_transparent():
    ...
    assert tooltip_image.getpixel((4, 4))[3] == 255
    assert tooltip_image.getpixel((16, 4))[3] == 255
    assert tooltip_image.getpixel((11, 4))[3] == 0
